...
- Four years of custom feature work and API integrations on a PHP/Laravel ecommerce
  platform, plus the merges that keep client installations upgradable
...
Moving deeper into infrastructure — cloud, containers, Kubernetes. I came up through
the application layer, and I want to understand the whole path from commit to
production, not just the part that runs inside a request cycle.

### Before software

Four years teaching robotics to kids (Lego Mindstorms, WeDo). Explaining a loop to a
ten-year-old turns out to be good preparation for explaining a regression to a client.
